...
exports.SharedBrain = SharedBrain;
const react_1 = require("react");
const react_router_dom_1 = require("react-router-dom");
const lucide_react_1 = require("lucide-react");
const content_1 = require("../api/content");
const utils_1 = require("../lib/utils"); // Assuming cn utility is available
// Import shadcn/ui components
const card_1 = require("@/components/ui/card"); // Using Card for content items
function SharedBrain() {
    const { sharelink } = (0, react_router_dom_1.useParams)();
    const [brainData, setBrainData] = (0, react_1.useState)(null);
    ...
    (0, react_1.useEffect)(() => {
        const fetchSharedBrain = () => __awaiter(this, void 0, void 0, function* () {
            if (!sharelink)
                return;
            try {
                const response = yield content_1.contentApi.getSharedBrain(sharelink);
    ...
    if (loading) {
        ...
        <card_1.Card className="glass-card p-12 flex flex-col items-center space-y-6 max-w-md">
    ...
    if (error || !brainData) {
        return (<div className="min-h-screen bg-background flex items-center justify-center">
        <card_1.Card className="glass-card p-12 flex flex-col items-center space-y-6 max-w-md">
          <lucide_react_1.Brain className="h-16 w-16 text-muted-foreground/50 mx-auto mb-4"/>
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-8 py-8">
        ...
        {brainData.content.length === 0 ? (<div className="text-center py-12">
            <card_1.Card className="glass-card p-16 max-w-2xl mx-auto">
            ...
            {brainData.content.map((content, index) => (<card_1.Card key={content._id} className="glass-card p-6 hover-lift hover-glow group relative overflow-hidden" style={{ animationDelay: `${index * 0.1}s` }}>
                <card_1.CardContent className="p-0 relative z-10">
                  ...
                  <div className="relative z-10">
                    ...
                    <a href={content.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-2 text-primary hover:text-primary-glow text-sm font-medium transition-colors hover:underline truncate">
                      <span className="truncate">Visit Link</span>
                      <lucide_react_1.ExternalLink className="h-4 w-4 flex-shrink-0"/>
                    </a>
                    {content.tags.length > 0 && (<div className="flex flex-wrap gap-2 mt-4">
                        {content.tags.map((tag, idx) => (<span key={idx} className="px-3 py-1 bg-muted/30 text-muted-foreground text-xs rounded-lg border border-muted/40">
                            {tag.title}
                          </span>))}
                      </div>)}
                  </div>
